    ...
    <div class="container-fluid px-0 px-md-4 py-0 py-md-4">
        ...
        <div class="d-lg-none sticky-top bg-white border-bottom shadow-sm z-3">
            ...
            <div class="d-flex overflow-auto px-3 pb-3 gap-2 no-scrollbar">
                <a href="{{ route('reports.index') }}"
                    class="btn btn-primary rounded-pill shadow-sm fw-bold px-4 flex-shrink-0">Sales</a>
                <a href="{{ route('reports.inventory') }}"
                    class="btn btn-light border rounded-pill px-4 text-secondary flex-shrink-0">Inventory</a>
                <a href="{{ route('reports.credits') }}"
                    class="btn btn-light border rounded-pill px-4 text-secondary flex-shrink-0">Credits</a>
                <a href="{{ route('reports.forecast') }}"
                    class="btn btn-light border rounded-pill px-4 text-secondary flex-shrink-0">Forecast</a>
            </div>
        </div>
        ...
        <div class="d-none d-lg-flex flex-column flex-xl-row justify-content-between align-items-xl-center mb-4 gap-3">
            ...
            <div class="d-flex flex-wrap gap-2">
                <a href="{{ route('reports.index') }}"
                    class="btn btn-primary shadow-sm flex-fill flex-xl-grow-0 rounded-pill fw-bold px-4 single-click-link">Sales</a>
                <a href="{{ route('reports.inventory') }}"
                    class="btn btn-white border shadow-sm flex-fill flex-xl-grow-0 rounded-pill px-4 text-secondary hover-primary single-click-link">Inventory</a>
                <a href="{{ route('reports.credits') }}"
                    class="btn btn-white border shadow-sm flex-fill flex-xl-grow-0 rounded-pill px-4 text-secondary hover-primary single-click-link">Credits</a>
                <a href="{{ route('reports.forecast') }}"
                    class="btn btn-white border shadow-sm flex-fill flex-xl-grow-0 rounded-pill px-4 text-secondary hover-primary single-click-link">Forecast</a>
            </div>
        </div>
        ...
        <div class="px-3 px-md-0 pt-3 pt-md-0">
            ...
            <div class="card mb-4 border-0 shadow-sm rounded-4">
                <div class="card-body p-2">
                    <form action="{{ route('reports.index') }}" method="GET" class="row g-2 align-items-center">
                                ...
                                <div class="col-12 col-md-3">
                                    <div class="input-group">
                                        <span class="input-group-text bg-light border-0"><i
                                                class="fas fa-store text-muted"></i></span>
                                        <select name="store_filter" class="form-select bg-light border-0"
                                            onchange="this.form.submit()">
                                            <option value="all" {{ $targetStore == 'all' ? 'selected' : '' }}>-- All Branches --
                                            </option>
                        ...
                        <div class="col-6 col-md-2">
                            <button type="submit" class="btn btn-dark w-100 rounded-pill shadow-sm"><i
                                    class="fas fa-filter me-1"></i> Filter</button>
                        </div>
                        <div class="col-6 col-md-2">
                            <a href="{{ route('reports.export', ['report_type' => 'sales', 'start_date' => $startDate]) }}"
                                class="btn btn-success w-100 rounded-pill shadow-sm text-white">
            ...
            <div class="row g-2 g-xl-3 mb-4">
                <div class="col-6 col-xl-3">
                    <div class="card h-100 shadow-sm border-0 rounded-4 overflow-hidden"
                        ...
                        <div class="card-body p-3 text-white">
                            <small class="text-white-50 text-uppercase fw-bold" style="font-size: 0.65rem;">Realized
                                Revenue</small>
                            ...
                            <div class="small text-white-50 mt-1" title="Sales excluding unpaid credits">
                                <i class="fas fa-wallet me-1 opacity-50"></i> vs ₱{{ number_format($total_sales, 2) }}
                                (Accrual)
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-6 col-xl-3">
                    <div class="card h-100 shadow-sm border-0 rounded-4 overflow-hidden"
                        ...
                        <div class="card-body p-3 text-white">
                            <small class="text-white-50 text-uppercase fw-bold" style="font-size: 0.65rem;">Gross
                                Profit</small>
                            <h3 class="fw-bold mb-0 mt-1">₱{{ number_format($gross_profit, 2) }}</h3>
                            <div class="small text-white-50 mt-1"><i class="fas fa-chart-line me-1 opacity-50"></i>Estimate
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card h-100 shadow-sm border-0 rounded-4 border-start border-4 border-warning bg-white">
                        <div class="card-body p-3">
                            <small class="text-muted text-uppercase fw-bold"
                                style="font-size: 0.65rem;">Transactions</small>
                            <h3 class="fw-bold mb-0 mt-1 text-dark">{{ number_format($total_transactions) }}</h3>
                            <div class="small text-muted mt-1"><i class="fas fa-receipt me-1 opacity-50"></i>Total Count
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-6 col-xl-3">
                    <div class="card h-100 shadow-sm border-0 rounded-4 overflow-hidden"
                        ...
                        <div class="card-body p-3 text-white">
                            <small class="text-white-50 text-uppercase fw-bold" style="font-size: 0.65rem;">Tithes
                                (10%)</small>
                            <h3 class="fw-bold mb-0 mt-1">₱{{ number_format($tithes_amount, 2) }}</h3>
                            <div class="small text-white-50 mt-1"><i
                                    class="fas fa-hand-holding-heart me-1 opacity-50"></i>Calculated</div>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover align-middle mb-0">
                                    ...
                                    <tbody>
                                        @forelse($topItems as $item)
                                            <tr>
                                                ...
                                                <td class="text-center"><span
                                                        class="badge bg-light text-dark border shadow-sm px-3 rounded-pill">{{ $item->total_qty }}</span>
                                                </td>
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="3" class="text-center py-5 text-muted">No sales items recorded for
                                                    this period.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                ...
                                @forelse($topItems as $index => $item)
                                    <div class="list-group-item p-3 border-0 bg-light rounded-4">
                                        <div class="d-flex align-items-center">
                                            <div class="bg-white rounded-circle shadow-sm d-flex align-items-center justify-content-center me-3 text-secondary fw-bold"
                                                style="width: 40px; height: 40px;">#{{ $index + 1 }}</div>
                                            <div class="flex-grow-1">
                                                <div class="fw-bold text-dark">{{ $item->product->name ?? 'Unknown' }}</div>
                                                <small class="text-muted">{{ $item->total_qty }} units sold</small>
                                            </div>
                                            <div class="text-end">
                                                <div class="fw-bold text-success">₱{{ number_format($item->total_revenue, 2) }}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            <div class="card shadow-sm border-0 mb-4 rounded-4 overflow-hidden">
                <div class="card-header bg-white py-3 border-bottom border-light">
                    <h5 class="m-0 font-weight-bold text-dark"><i class="fas fa-receipt me-2 text-primary"></i>Recent
                        Transactions</h5>
                </div>
                        ...
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                @forelse($sales as $sale)
                                    <tr>
                                        <td class="ps-4 text-muted"><span
                                                class="badge bg-light text-dark border rounded-pill px-3">#{{ $sale->id }}</span>
                                        </td>
                                        <td>{{ $sale->created_at->format('M d, h:i A') }}</td>
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <div class="bg-light rounded-circle me-2 d-flex justify-content-center align-items-center text-primary"
                                        ...
                                        <td>
                                            ...
                                            @endphp
                                            <span
                                                class="badge bg-{{ $methodColor }}-subtle text-{{ $methodColor }} border border-{{ $methodColor }}-subtle rounded-pill text-uppercase px-3">
                                                {{ $sale->payment_method }}
                                            </span>
                                        </td>
                                        ...
                                        <td class="text-end pe-4">
                                            <a href="{{ route('transactions.show', $sale->id) }}"
                                                class="btn btn-sm btn-light border text-primary fw-bold rounded-pill px-3 shadow-sm hover-lift">
                                                View
                                            </a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="6" class="text-center py-5 text-muted">No transactions found for this
                                            period.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                            ...
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    ...
                                    <small class="text-muted"><i
                                            class="far fa-clock me-1"></i>{{ $sale->created_at->format('M d, h:i A') }}</small>
                                </div>
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    ...
                                    @endphp
                                    <span
                                        class="badge bg-{{ $methodColor }}-subtle text-{{ $methodColor }} rounded-pill text-uppercase border border-{{ $methodColor }}-subtle">
                                        {{ $sale->payment_method }}
                                    </span>
                                </div>

                                <div class="mt-3 pt-3 border-top d-flex justify-content-between align-items-center">
                                    <span class="small text-muted fw-bold"><i class="fas fa-user me-1"></i>
                                        {{ $sale->customer->name ?? 'Walk-in' }}</span>
                                    <a href="{{ route('transactions.show', $sale->id) }}"
                                        class="btn btn-sm btn-light border rounded-pill px-3 fw-bold text-primary">Details</a>
                                </div>
                            </div>
                        </div>
                    @empty
