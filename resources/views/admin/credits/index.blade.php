...
@section('content')
        ...
        <div class="d-none d-lg-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('credits.logs') }}"
                    class="btn btn-light border shadow-sm rounded-pill fw-bold flex-fill flex-md-grow-0">
                    <i class="fas fa-history me-1 text-secondary"></i> Logs
                </a>
                <a href="{{ route('credits.export') }}"
                    class="btn btn-success shadow-sm rounded-pill fw-bold px-4 flex-fill flex-md-grow-0">
            ...
            <div class="card-body p-3">
                <form action="{{ route('credits.index') }}" method="GET" class="row g-2 align-items-center">
                    ...
                    <div class="col-6 col-md-3">
                        <select name="sort" class="form-select bg-light border-0 py-2 text-secondary fw-bold"
                            style="cursor: pointer;">
                            <option value="newest">Newest First</option>
                        </select>
                    </div>
                    <div class="col-6 col-md-3">
                        <select name="filter" class="form-select bg-light border-0 py-2 text-secondary fw-bold"
                            style="cursor: pointer;">
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($credits as $credit)
                                <tr>
                                    ...
                                    <td class="text-end text-danger fw-bold fs-6">
                                        ₱{{ number_format($credit->remaining_balance, 2) }}</td>
                                    <td class="ps-4">
                                        @if($credit->due_date)
                                            @if(\Carbon\Carbon::parse($credit->due_date)->isPast())
                                                <span
                                                    class="badge bg-danger-subtle text-danger border border-danger-subtle rounded-pill"><i
                                                        class="fas fa-exclamation-circle me-1"></i>Overdue</span>
                                            @else
                                                <span
                                                    class="badge bg-light text-secondary border rounded-pill">{{ \Carbon\Carbon::parse($credit->due_date)->format('M d') }}</span>
                                            @endif
                                        @else
                                    ...
                                    <td class="text-end pe-4">
                                        <div class="btn-group shadow-sm rounded-pill">
                                            @if(auth()->user()->role !== 'auditor')
                                                <a href="{{ route('credits.pay_form', $credit->credit_id) }}"
                                                    class="btn btn-sm btn-success fw-bold px-3">
                                                    <i class="fas fa-wallet me-1"></i> Pay
                                                </a>
                                            @endif
                                            <a href="{{ route('credits.history', $credit->credit_id) }}"
                                                class="btn btn-sm btn-light border-start text-secondary px-3"
                                                title="View History">
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center py-5 text-muted">No outstanding credits found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <ul class="list-group list-group-flush">
                @forelse($credits as $credit)
                    <li class="list-group-item p-3 border-bottom-0 hover-bg-light"
                        onclick="openCreditActionSheet('{{ $credit->credit_id }}', '{{ addslashes($credit->customer->name ?? 'Unknown') }}', '{{ $credit->remaining_balance }}', '{{ route('credits.pay_form', $credit->credit_id) }}', '{{ route('credits.history', $credit->credit_id) }}')"
                        style="cursor: pointer;">
                                ...
                                <div>
                                    ...
                                    @if($credit->due_date && \Carbon\Carbon::parse($credit->due_date)->isPast())
                                        <span class="badge bg-danger rounded-pill shadow-sm"
                                            style="font-size: 0.65rem;">Overdue</span>
                                    @else
                                        <small class="text-muted" style="font-size: 0.75rem;">Due:
                                            {{ \Carbon\Carbon::parse($credit->due_date)->format('M d') }}</small>
                                    @endif
                                </div>
                            </div>
                            <div class="text-end">
                                <h6 class="fw-bold text-danger mb-0">₱{{ number_format($credit->remaining_balance, 2) }}</h6>
                                <small class="text-success" style="font-size: 0.75rem;">Paid:
                                    ₱{{ number_format($credit->amount_paid, 2) }}</small>
                            </div>
                        </div>
                    </li>

                    {{-- Action Sheet moved to bottom --}}
    ...
    {{-- Placed outside of overflow-hidden containers to prevent backdrop/z-index issues --}}
    {{-- Placed once to prevent Duplicate ID issues --}}
    {{-- UNIFIED MODALS SECTION --}}
    {{-- Placed outside of overflow-hidden containers to prevent backdrop/z-index issues --}}
    {{-- Placed once to prevent Duplicate ID issues --}}
    {{-- UNIFIED ACTION SHEET (Mobile) --}}
    <div class="modal fade modal-bottom-sheet" id="creditActionSheet" tabindex="-1" aria-hidden="true">
            ...
            <div class="modal-content bg-transparent shadow-none backdrop-blur-0">
                ...
                <div class="bg-surface px-3 pb-4 pt-2 rounded-top-5">
                    ...
                    <div class="text-center mb-4">
                        <h5 class="fw-bold text-dark m-0" id="sheetCustomerName">Customer Name</h5>
                        <p class="text-muted small m-0">Balance: <span class="text-danger fw-bold"
                                id="sheetBalance">₱0.00</span></p>
                    </div>

                    <div class="mobile-action-group shadow-sm">
                        @if(auth()->user()->role !== 'auditor')
                            <a href="#" id="sheetPayBtn" class="mobile-action-btn text-decoration-none">
                        ...
                        <a href="#" id="sheetHistoryBtn" class="mobile-action-btn text-decoration-none">
    ...
    <script>
        function openCreditActionSheet(id, name, balance, payUrl, historyUrl) {
            document.getElementById('sheetCustomerName').innerText = name;
            // Format balance with commas
            const formattedBalance = parseFloat(balance).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
            document.getElementById('sheetBalance').innerText = '₱' + formattedBalance;

            const payBtn = document.getElementById('sheetPayBtn');
            if (payBtn) payBtn.href = payUrl;

            document.getElementById('sheetHistoryBtn').href = historyUrl;

            new bootstrap.Modal(document.getElementById('creditActionSheet')).show();
        }
    </script>
