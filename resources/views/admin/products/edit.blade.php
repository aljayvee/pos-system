...
@section('content')
    <div class="container-fluid px-0 px-md-4 py-0 py-md-4 bg-light h-100">
        ...
        {{-- MOBILE HEADER --}}
        <div
            class="d-lg-none sticky-top bg-white border-bottom shadow-sm px-3 py-3 d-flex align-items-center justify-content-between z-3">
        ...
        <form id="editProductForm" action="{{ route('products.update', $product->id) }}" method="POST"
            enctype="multipart/form-data" class="pb-5 mb-5 pb-lg-0 mb-lg-0">
            @csrf
            @method('PUT')
            ...
            <div class="row g-0 g-lg-4">
                    ...
                    <div class="d-lg-none bg-white p-4 text-center border-bottom mb-3">
                        <div class="position-relative d-inline-block">
                            <div class="rounded-4 d-flex align-items-center justify-content-center bg-light border"
                                ...
                                @if($product->image)
                                    <img id="mobileImagePreview" src="{{ asset('storage/' . $product->image) }}" alt="Preview"
                                ...
                                @else
                                    <img id="mobileImagePreview" src="#" alt="Preview"
                        ...
                        <input type="file" id="mobileImageInput" name="image" class="d-none" accept="image/*"
                            onchange="previewImage(this, 'mobileImagePreview', 'mobilePlaceholderIcon')">
                    </div>
                    ...
                    <div class="card shadow-sm border-0 rounded-0 rounded-lg-4 overflow-hidden mb-3 mb-lg-4 mx-0 mx-md-0">
                        ...
                        <div class="card-body p-3 p-lg-4">
                            ...
                            <div class="mb-3 mb-lg-4">
                                <label class="form-label fw-bold small text-secondary d-none d-lg-block">PRODUCT NAME <span
                                        class="text-danger">*</span></label>
                                <input type="text" name="name"
                                    class="form-control bg-light border-0 py-3 fw-bold text-dark fs-5"
                            ...
                            <div class="row g-2 g-lg-4 mb-3 mb-lg-4">
                                <div class="col-12 col-md-5">
                                    <label class="form-label fw-bold small text-secondary d-none d-lg-block">Category <span
                                            class="text-danger">*</span></label>
                                    <div class="input-group">
                                        <select name="category_id" class="form-select bg-light border-0 fw-bold py-3"
                                        ...
                                        <button type="button" class="btn btn-primary px-3 shadow-none"
                                ...
                                <div class="col-6 col-md-4">
                                    <label class="form-label fw-bold small text-secondary d-none d-lg-block">Tax Type <span
                                            class="text-danger">*</span></label>
                                    <select name="tax_type" class="form-select bg-light border-0 fw-bold py-3" required>
                                        <option value="vatable" {{ $product->tax_type == 'vatable' ? 'selected' : '' }}>
                                            Vatable</option>
                                        <option value="vat_exempt" {{ $product->tax_type == 'vat_exempt' ? 'selected' : '' }}>
                                            VAT Exempt</option>
                                        <option value="zero_rated" {{ $product->tax_type == 'zero_rated' ? 'selected' : '' }}>
                                            Zero Rated</option>
                                    </select>
                                </div>
                                @else
                                <input type="hidden" name="tax_type" value="{{ $product->tax_type }}">
                                @endif

                                <div class="col-6 col-md-3">
                                    <label class="form-label fw-bold small text-secondary d-none d-lg-block">Unit</label>
                                    <input type="text" name="unit" class="form-control bg-light border-0 fw-bold py-3"
                                        list="unitOptions" value="{{ $product->unit }}" placeholder="e.g. Pc" required>
                            ...
                            <div class="mb-2 d-none d-lg-block mt-4">
                                ...
                                <div class="d-flex align-items-center gap-3 p-3 border-0 bg-light rounded-4">
                                    <div class="rounded-4 d-flex align-items-center justify-content-center bg-white shadow-sm"
                                        ...
                                        @if($product->image)
                                            <img id="desktopImagePreview" src="{{ asset('storage/' . $product->image) }}"
                                                alt="Product" style="width: 100%; height: 100%; object-fit: cover;">
                                            <i id="desktopPlaceholderIcon" class="fas fa-image text-muted fa-2x"
                                                style="display: none;"></i>
                                        @else
                                            <img id="desktopImagePreview" src="#" alt="Preview"
                                                style="display: none; width: 100%; height: 100%; object-fit: cover;">
                                            <i id="desktopPlaceholderIcon" class="fas fa-image text-muted fa-2x"></i>
                                        @endif
                                    </div>
                                    <div class="flex-grow-1">
                                        <input type="file" id="desktopImageInput" name="image"
                                            class="form-control bg-white border-0 shadow-sm" accept="image/*"
                                            onchange="previewImage(this, 'desktopImagePreview', 'desktopPlaceholderIcon')">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-lg-4">
                        ...
                        <div class="card shadow-sm border-0 rounded-0 rounded-lg-4 mb-3 mb-lg-4">
                            <div class="card-header bg-white py-3 border-bottom-0 d-none d-lg-block">
                                <h5 class="mb-0 text-dark fw-bold"><i class="fas fa-barcode me-2 text-secondary"></i>Barcode
                                </h5>
                            </div>
                            <div class="card-body p-3 p-lg-4">
                                <div class="mb-3">
                                    ...
                                    <div class="input-group shadow-sm rounded-3 overflow-hidden">
                                        ...
                                        <input type="text" name="sku" class="form-control bg-white border-0 fw-bold py-3"
                                            value="{{ $product->sku }}">
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card shadow-sm border-0 rounded-0 rounded-lg-4 mb-3 mb-lg-4">
                            <div class="card-header bg-white py-3 border-bottom-0 d-none d-lg-block">
                                <h5 class="mb-0 text-dark fw-bold"><i
                                        class="fas fa-warehouse me-2 text-warning"></i>Inventory</h5>
                            </div>
                            <div class="card-body p-3 p-lg-4">
                                <div class="mb-3">
                                    <label class="form-label fw-bold small text-secondary">Current Stock</label>
                                    <input type="number" name="stock"
                                        class="form-control bg-light border-0 text-muted p-3 fw-bold"
                                        value="{{ $product->stock }}" readonly disabled style="cursor: not-allowed;">
                                    <div class="form-text small text-warning mt-2"><i
                                            class="fas fa-exclamation-circle me-1"></i> Stock levels are managed via
                                        <strong>Purchase History</strong> or <strong>Stock Adjustments</strong>.</div>
                                </div>
                                <div class="row g-2">
                                    <div class="col-12 mb-3">
                                        <label class="form-label fw-bold small text-secondary">Reorder Pt</label>
                                        <input type="number" name="reorder_point"
                                            class="form-control bg-light border-0 p-3 fw-bold"
                        ...
                        <div class="d-none d-lg-grid gap-2 mt-4">
                            <button type="button" id="btn-update-desktop" onclick="validateAndUpdate({{ $product->id }})"
                                class="btn btn-warning btn-lg shadow-lg rounded-pill fw-bold text-dark">
                                <i class="fas fa-save me-2"></i> Update Product
                            </button>
                            <a href="{{ route('products.index') }}"
                                class="btn btn-light rounded-pill text-muted fw-bold">Cancel Changes</a>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-lg-none px-3 mt-4 mb-5">
                    <button type="button" id="btn-update-mobile" onclick="validateAndUpdate({{ $product->id }})"
                        class="btn btn-primary w-100 rounded-pill fw-bold py-3 text-uppercase ls-1 shadow-sm">
                        Update Product
                    </button>
                </div>
        </form>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg rounded-4">
                ...
                <div class="modal-body p-4 text-center">
                    ...
                    <p class="text-muted mb-0">You cannot use this Name or SKU because it is already taken by another
                        product.</p>
                </div>
